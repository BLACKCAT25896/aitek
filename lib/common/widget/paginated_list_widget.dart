...
class PaginatedListWidget extends StatefulWidget {
  ...
  const PaginatedListWidget({
    ...
    required this.offset, required this.itemView, this.enabledPagination = true,
    this.reverse = false,
  });
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      ...
      (ResponsiveHelper.isDesktop(context) && (widget.totalSize == null ||
          _offset! >= (widget.totalSize! / 10).ceil() || _offsetList.contains(_offset!+1))) ?
      const SizedBox() : Center(child: Padding(
        padding: (_isLoading || ResponsiveHelper.isDesktop(context)) ?

        const EdgeInsets.all(Dimensions.paddingSizeSmall) : EdgeInsets.zero,
        ...
        (ResponsiveHelper.isDesktop(context) && widget.totalSize != null) ? InkWell(
          ...
          child: Container(padding: const EdgeInsets.symmetric(
            ...
            child: Text("view_more".tr, style: textRegular.copyWith(
